...
if($work_type_require == 'and'):
            ...
            if($param->domen_is):
                ...
                if($param->domen_is == 'only' && !modMultiFormHelper::inArray ($_SERVER['HTTP_HOST'],$domen_sites,'www.')){
                    //throw new Exception('only');
                    return FALSE;
                }
                else if($param->domen_is == 'without' && modMultiFormHelper::inArray ($_SERVER['HTTP_HOST'],$domen_sites,'www.')){
            ...
            if($param->ip_user_is):
                ...
                if($param->ip_user_is == 'only' && !modMultiFormHelper::inArray($_SERVER['REMOTE_ADDR'], $ips_user)){
                    return FALSE;
                }
                else if($param->ip_user_is == 'without' && modMultiFormHelper::inArray($_SERVER['REMOTE_ADDR'], $ips_user)){
                    return FALSE;
                }
            endif;
            ...
            if($param->view_is):
                ...
                elseif($param->view_is == 'only' &&  !modMultiFormHelper::inArray($view_site, $views)){ 
                    return FALSE; 
                }
                else if($param->view_is == 'without' &&  modMultiFormHelper::inArray($view_site, $views)){ 
                    return FALSE;
                }
            endif;
