...
scrollFromTop();positiOnScroll=50}else{scrollFT=0;positiOnScroll=heightpage/2-heightpopupblock/2+scrollFT}let modal=jQuery("#"+modal_id);console.log(modal,"HaHa");modal.css("display","block").animate({opacity:1,top:positiOnScroll,opacity:1,visibility:"visible"},400,()=>{let overlay=jQuery(this).next(".mfOverlay");console.log(overlay,"HaHa 2");jQuery(this).modal("show").css("padding",0).css("padding-right",0);console.log(overlay,"HaHa 4")}).css("opacity",1).css("top","200")});return false}
...
function mfButtonSubmit_Click(e){let params=e.data;if(e.first)e.preventDefault();let func_custom=window["funcBefore"+params.id];if(func_custom&&typeof func_custom==="function")func_custom.apply(params);jQuery("#mfForm_form_"+params.id).validate({debug:false,invalidHandler:function(event,validator){console.log("\ud83d\udc4e _validator");console.log("\ud83d\udc4e _validator",validator)},submitHandler:function(form){e.preventDefault();console.log("(I)  :-)  - submitHandlerValidate!!!!!->  ()  Captcha:",
params.captcha?"Yes\ud83c\udf1f":"No\ud83d\udeab",params.captcha,params);if(params.captcha=="recaptcha"&&params.grecaptcha!==false){let rdy=this;params.response=grecaptcha.getResponse(params.WidgetId);if(params.response){console.log("(II) Execute() -submitHandler-CallBack!!!!!-> Token"," This:",this," Module:",params);submitHandler.call(params,form);if(params.xCallback&&params.xCallback.trim())try{let callback=params.xCallback.trim();eval(callback+"()")}catch(e){}}return false}if(params.captcha==
"recaptcha_invisible"&&params.grecaptcha!==false){let rdy=this;grecaptcha.execute(params.grecaptcha).then(function(token){params.token=token;console.log("(II) Execute() -submitHandler-CallBack!!!!!-> Token",token," This:",this," Module:",params);return;submitHandler.call(params,form)});return}else if(params.captcha=="recaptcha_invisible"&&params.grecaptcha===false){console.log("(II) Valid() -submitHandler-CallBack!!!!!-> "," This:",this," Module:",params);return}else if(!params.captcha||params.grecaptcha===
false){console.log("(II) Valid() -submitHandler-CallBack!!!!!-> "," This:",this," Module:",params);submitHandler.call(params,form);return}else;}})}
function submitHandler(form){console.log("(III)  submitHandler() -Execute-CallBack!!!!!-> ",this);jQuery("#submit_"+this.id).button("sending").css("text-transform","none").css("transform","none").addClass("active");let itemid=0;for(let clss of document.body.classList)if(clss.toString().substr(0,7)==="itemid-")itemid=clss.toString().substr(7);var data=new FormData(form);data.append("option","com_ajax");data.append("module","multi_form");data.append("format","raw");data.append("id",this.id);data.append("page",
document.location.href);data.append("title",document.title);data.append("Itemid",itemid);if(this.token)data.set("gToken",this.token);if(this.response)data.set("gToken",this.response);if(this.sitekey!==undefined)data.append("sitekey",this.sitekey);if(this.grecaptcha!==undefined)data.append("grecaptcha",this.grecaptcha);if(this.response)data.append("g-recaptcha-response",this.response);console.log("---------");for(let field of this.fields)if(field.substring(0,4)==="file")for(let file of document.getElementById(field).files)data.append(field,
file);else;let url=window.location.origin+"/index.php";url=document.baseURI+"index.php";this.deb&&console.log("Ajax request ModuleID:",this.id," ",data);jQuery.ajax({type:"POST",url:url,dataType:"html",data:data,context:this,cache:false,contentType:false,processData:false}).done(mfAjaxDoneSuccess).fail(mfAjaxFailForm)}
...
function mfAjaxFailForm(jqXHR,status,errorThrown){console.clear();this.deb&&console.log("\ud83d\udc4e Module id:"+this.id+" Tag:"+this.tag+" Type:"+this.type+" - Load form Fail! - Disabled button! status:",status,"\n this:",this," jqXHR:",jqXHR,"\n errorThrown:",errorThrown);jQuery("#"+this.button).hide()}
