...
//		shm_put_var($shmId, 'modMultiForm.'.$modID.'.message.'.$orderID, $message);
	...
	shm_put_var($shmId, 0, $status);    
//		if (shm_has_var($shmId, 0)) {
//			// Если есть, считываем данные
//			$status = (string)shm_get_var($shmId, 0);
//			shm_remove($shmId);
//		}
//		sem_release($semId);
//		sem_remove($semId);
...
<!DOCTYPE html>
<title>SUBMIT</title>
<h1><a href="//<?=$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']?>">SUBMIT</a> - <?= date('Y-m-d H:i:s');?></h1>
<br>

<pre> ModID:   <?=$modID?></pre><br><br>
<pre> OrderID: <?=$orderID?></pre><br><br>
<pre> Status:  <?=$status?></pre><br><br>
...
<form>
	Status: <input name="mod" type="text" value="<?= $_REQUEST['mod']??''?>" >
	Order: <input name="order" type="text" value="<?= $_REQUEST['order']??''?>" >
	Status: <input name="status" type="text" value="<?= $_REQUEST['status']??''?>" >
